...
2. Only stop early for a true blocker (missing required auth/permissions/secrets). Signal blockers by writing a short message to `$HIVEBOARD_QUESTION` and exiting — not via prose in your final message, which is not parsed.
...
6. `git` and `gh` are pre-authenticated against `{{ task.repo_owner }}/{{ task.repo_name }}` via the host orchestrator. Just run `git push` and `gh pr create`. Do not set `GITHUB_TOKEN`/`GH_TOKEN` or call `gh auth login` — that would freeze a stale token and defeat the host's mid-run credential refresh. On HTTP 401 do not retry — signal via `$HIVEBOARD_QUESTION` (per rule 2) and exit.
