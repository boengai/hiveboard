...
describe('GitHubClient token file propagation', () => {
  const origEnv = { ...process.env }
  const tokenDir = join(tmpdir(), `hiveboard-tokens-${process.pid}`)
  ...
  it('writes token files on first getAccessToken() call (PAT)', async () => {
    process.env.GITHUB_TOKEN = 'ghp_test_token_file'
    const { GitHubClient } = await import('../src/github/client')
    const client = GitHubClient.create()

    await client.getAccessToken()

    const dir = client.getTokenDir()
    expect(existsSync(join(dir, 'token'))).toBe(true)
    expect(readFileSync(join(dir, 'token'), 'utf-8')).toBe(
      'ghp_test_token_file',
    )

    expect(existsSync(join(dir, 'askpass.sh'))).toBe(true)
    const askpass = readFileSync(join(dir, 'askpass.sh'), 'utf-8')
    expect(askpass).toContain('HIVEBOARD_TOKEN_FILE')

    expect(existsSync(join(dir, 'gh', 'hosts.yml'))).toBe(true)
    const hosts = readFileSync(join(dir, 'gh', 'hosts.yml'), 'utf-8')
    expect(hosts).toContain('ghp_test_token_file')
  })
})
