...
6. `git` and `gh` are pre-authenticated against `{{ task.repo_owner }}/{{ task.repo_name }}` (`GITHUB_TOKEN`/`GH_TOKEN` are set). Just run `git push` and `gh pr create`. On HTTP 401 do not retry — signal via `$HIVEBOARD_QUESTION` (per rule 2) and exit.
