...
function renameColumn(db: Database, table: string, oldName: string, newName: string): void {
  const cols = db.query(`PRAGMA table_info('${table}')`).all() as Array<{ name: string }>
  if (cols.some((c) => c.name === oldName) && !cols.some((c) => c.name === newName)) {
...
function addMissingColumns(db: Database): void {
  const user = db.query("SELECT id FROM users WHERE username = 'queen-bee' LIMIT 1").get() as { id: string } | null

  // boards.created_by
  if (ensureColumn(db, 'boards', 'created_by', 'TEXT REFERENCES users(id)') && user) {
    db.run('UPDATE boards SET created_by = ? WHERE created_by IS NULL', [user.id])
  }
