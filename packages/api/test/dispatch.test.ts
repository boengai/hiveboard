...
afterEach(() => {
  ...
  memDb.run(
    "DELETE FROM task_events WHERE type IN ('action_set', 'status_changed')",
  )
})
...
describe('dispatchAgent – target_repo requirement', () => {
  ...
  it('throws for implement-e2e when target_repo is null', () => {
    const id = insertTask({ targetRepo: null })
    expect(() => dispatchAgent(id, 'implement-e2e')).toThrow(
      /requires target_repo/,
    )
  })
...
describe('dispatchAgent – happy path', () => {
  ...
  it('inserts action_set and status_changed events', () => {
    ...
    const events = memDb
      .query(
        'SELECT type FROM task_events WHERE task_id = ? ORDER BY created_at ASC',
      )
