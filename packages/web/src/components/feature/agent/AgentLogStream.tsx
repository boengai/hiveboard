...
export function AgentLogStream({ taskId, agentStatus }: AgentLogStreamProps) {
  ...
  useEffect(() => {
    const el = scrollRef.current as (HTMLDivElement & { scrollTop: number; scrollHeight: number }) | null
    if (el) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border-default bg-surface-inset p-3">
      ...
      <div className="flex items-center justify-between">
        <span className="text-body-xs font-medium text-text-secondary">Agent Output</span>
